...
>
> **Addendum 6 (2026-07-14/15):** branch `feat/silk-hero-background-assets` delivered the base assets for an **interactive silk hero background** (spec: `docs/superpowers/specs/2026-07-14-silk-hero-interactive-background-design.md`, plan: `docs/superpowers/plans/2026-07-14-silk-hero-interactive-background.md`, execution deferred to a separate session on `feat/silk-hero-interactive-background`). Key facts: the client hero photo (`hero.jpg`, **1555×1012**) failed the ≥3840×2160 sharpness floor for a fullscreen Retina hero — it is why the current hero reads blurry — and a 5K SwiftShader capture of the retired procedural silk shader proved the shader look was never calibrated (chaotic noise; rejected). The base texture was instead **rendered offline** by the new deterministic renderer `scripts/render-silk-still.mjs`; the client selected variant **V3 "corner drape"** with a darker premium grade (shadows `#38200a` → base `#8f571c` → peak champagne `#deb37a`, sampled from and biased below the approved photo). Delivered tiers in `public/images/hero/silk/`: `silk-5120.jpg` / `silk-3840.jpg` / `silk-2048.jpg` (q95/q95/q92, all 4:4:4) — **these must be served as-is from `/public`, never through `next/image` or any optimizer.** The homepage itself is unchanged by this branch; the backdrop swap happens in the execution session.
>
...
>
> **Addendum 7 (2026-07-15):** plan `docs/superpowers/plans/2026-07-14-silk-hero-interactive-background.md` (branch `feat/silk-hero-interactive-background`, spec `docs/superpowers/specs/2026-07-14-silk-hero-interactive-background-design.md`) executed on top of Addendum 6's base assets: the Hero backdrop is now an **interactive silk fabric WebGL background** (`src/components/silk/fabric/`) — a texture-mapped, subdivided plane rendering the client-selected 5K "corner drape" still with subtle cursor-driven vertex displacement and a lighting-only "delta sheen"; the photographic texture always dominates, and no blur/bloom/grain/ripples/glow are applied (client brief hard rule). Gating mirrors the retired `SilkHero` (`src/components/silk/`, still un-mounted, recoverable): no WebGL2 / `prefers-reduced-motion` / slow connection / `NEXT_PUBLIC_SILK_HERO=off` kill switch / context loss all resolve to the same static poster the live canvas renders at rest. The static `hero.jpg` (1555×1012, Addendum 6) remains in the repo as the client-approved reference photo but is **no longer rendered** — nothing else in Hero.tsx changed. The silk textures in `public/images/hero/silk/` must never go through `next/image` or any optimizer; this was re-verified on the deployed Vercel preview (`node scripts/verify-silk-quality.mjs <preview-url>`), though the byte-identity check against the SSO-gated preview needed the user's authenticated browser session rather than an unauthenticated `curl`. Quality gates: `npm run test`/`typecheck`/`lint`/`build` all green; `scripts/verify-silk-quality.mjs` (resolution/no-stretch matrix, six-viewport) passes; a real, non-headless GPU measurement on this Mac (not the non-representative SwiftShader headless numbers) showed 60fps, display-refresh-capped, no adaptive quality ladder needed. Still open: `docs/superpowers/specs/2026-07-14-silk-hero-interactive-background-design.md`'s art-direction taste dials (`src/components/silk/fabric/fabric.config.ts`) are a first pass, not yet Sofia-reviewed against the approved photo's exact grade.
>
> **Addendum 8 (2026-07-16):** plan `docs/superpowers/plans/2026-07-16-map-cert-swipe-why-imperium.md` (branch `fix/map-cert-swipe-why-imperium`) landed four client changes: (1) the Why Imperium route map was replaced with the client's new artwork (`/images/map/italy-gulf-routes-v2.png`; old file deleted, recoverable via git) — note the delivered PNG does **not** actually carry an alpha channel despite being described as "transparent background" (its corners are opaque near-white, which happens to match the Gesso `#FFFFFF` section band closely enough to look transparent today; a future non-white background behind this figure would reveal a hard rectangle — flagged for a docs/asset correction, not fixed this pass); (2) the diploma scan was **seamlessly redacted** — Sofia's DOB/birthplace line removed by the deterministic `scripts/redact-certification.mjs` and reissued as `made-in-italy-diploma.png` (old URL dead; visually confirmed with before/after/control crops; ⚠️ the unredacted original remains in the public repo's git history — separate follow-up task, user decision 2026-07-16); (3) mobile Collections swiping: the iOS long-press image callout on card media was suppressed and TiltCard's touch press-scale was removed (MOTION_SPEC §3.1 amended, though lines 102/118 nearby are now stale and describe touch handlers that no longer exist — doc nit for a later pass) — **the plan's real-device checkpoint (Step 7) was not completed**: the user was asked twice in-session whether the fix felt fluid on a real phone and did not respond before saying "continue," so the conditional snap-relaxation/vertical-stack fallback steps were not applied; this is an open item, re-test on a phone and report back if it still feels sticky; (4) Why Imperium row headings gained a fine gold underline (`text-underline-offset: 0.3em` starting value, not further tuned — no browser tool was available in this execution session to visually tune it live) and item 01 gained a competitive direct-sourcing sentence (no price mention, verified; AR machine draft added to the review sheet, §10 item 6 gate applies) — **new EN sentence still needs Sofia's sign-off**. Separately, this session discovered and stopped a stray leftover git worktree (`.claude/worktrees/agent-a6548e3c4aee855b7`, on unrelated branch `fix/silk-hero-visible-cloth-warp`) that had a `next dev` server running from a prior session, bound to the LAN and serving the **pre-redaction, unredacted diploma** the whole time — it was killed and a correct dev server restarted from this branch's actual working tree; the worktree itself was left untouched (not this plan's scope, may be in-progress work) but its presence is worth Akshay/the user cleaning up (`git worktree remove` once confirmed unneeded) since a stray worktree serving stale/sensitive assets on the LAN is a real hazard, not just clutter.
...
10. ✅ **RESOLVED (client, 2026-07-14) — lead inbox.** Displayed and lead-routed email is now `imperium.italian.textile@gmail.com` (interim Gmail, client-confirmed; reverts to `hello@imperiumitaliantextile.com` once the domain is registered and Resend-verified — a two-line change, PRD D-07).
11. Instagram `@imperiumitaliantextile` — existence still unverified.
12. ✅ **RESOLVED (client, 2026-07-14) — WhatsApp-first strategy.** Inline button + persistent 56px mobile bar, both fully built and live. Real WhatsApp Business number `971544844082` wired as the code fallback (client-confirmed); verify the Vercel env var isn't overriding it with the old dummy value.
...
| `NEXT_PUBLIC_WHATSAPP_NUMBER`                  | WhatsApp CTA                                     | real number `971544844082` as fallback and in `.env.example` (client-confirmed 2026-07-14)              |
...
2. ✅ **`/ar` infinite redirect loop — superseded, not just fixed.** The original note here (`next.config.ts` no longer contains the `/ar` redirect) described removed groundwork while the EN/AR decision was still open. That decision has since been made (§12 item 4): real locale routing now exists via a `[locale]` route segment plus middleware — a **rewrite** for `en` (no visible prefix) and a **visible `/ar` prefix** for Arabic, both resolved from a single canonical path, not a redirect chain. This is deliberate architecture, not the old removed groundwork rebuilt by accident, and it carries no loop risk.
...
6. **NEW, hard launch gate — AR copy is a machine-assisted draft, not native-reviewed.** `/ar` is now fully built (§12 item 4) and every Arabic string is real, structurally complete copy (guarded by `tests/unit/data/locale-parity.test.ts`), but it is MSA drafted by the build agent, not vetted by a native Gulf-register speaker. Before AR can go live to real traffic: (1) a native Arabic speaker reviews and corrects every row in `docs/superpowers/specs/2026-07-14-ar-copy-review-sheet.md`, (2) Sofia signs off on the corrected copy, (3) corrections land in the `ar` keys of `src/data/*.ts`. The privacy policy additionally needs professional legal translation, not just native copy review. Low urgency only because the site remains noindex (§10 item 1) — this becomes urgent the moment `NEXT_PUBLIC_ALLOW_INDEXING` flips to `"true"`.
...
4. ~~Language: EN only or EN+AR?~~ — **resolved: EN+AR, implemented.** The full site now works in Arabic at `/ar` — a `[locale]` route segment + middleware (rewrite for `en`, visible prefix for `ar`; see §10 item 2), localized Amiri/IBM Plex Sans Arabic fonts, RTL-mirrored layout (including the scroll-driven Collections showcase), and localized metadata/sitemap/hreflang. Every AR string is a **machine-assisted MSA draft**, not native-reviewed — see the review sheet at `docs/superpowers/specs/2026-07-14-ar-copy-review-sheet.md` and the new hard launch gate at §10 item 6: native Arabic speaker review (Gulf market register) + Sofia's sign-off on the corrected copy is required before AR traffic goes live, and the privacy policy needs separate professional legal translation.
5. **Inquiry channel operational readiness:** ~~destination inbox confirmation~~ and ~~real WhatsApp Business number~~ resolved (client, 2026-07-14 — `imperium.italian.textile@gmail.com` and `971544844082`). Still missing: who actually answers within the promised "one business day" (`contact.ts` copy).
